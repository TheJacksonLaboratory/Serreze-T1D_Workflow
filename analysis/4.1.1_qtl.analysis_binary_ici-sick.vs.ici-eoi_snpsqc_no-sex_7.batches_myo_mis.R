---
title: "QTL Analysis - binary [ICI Sick vs ICI EOI]"
...
+ Covariates (Myocarditis Score, Age of Clinical Phenotype– if Null, either use 40 age at which surviving mice were end of incidence without clinical symptoms, sex)
...
#gm$covar$ici.sick_vs_ici.eoi <- ifelse(gm$covar$group == "PBS", 0, 1)
#gm.full <- gm

covars <- read_csv("data/covar_corrected_ici-sick.vs.ici-eoi_7.batches_myo_mis.csv")
#removing any missing info
#covars <- subset(covars, covars$ici.sick_vs_ici.eoi!='')
nrow(covars)
...
#dismark <- read.csv("data/ici-sick.vs.ici-eoi_marker.freq_low.geno.freq.removed_geno.ratio_7.batches_myo_mis.csv")
#nrow(dismark)
...
covars$ici.sick_vs_ici.eoi= as.numeric(covars$ici.sick_vs_ici.eoi)

kinship <- calc_kinship(pr.qc)
heatmap(kinship)

table(covars["ici.sick_vs_ici.eoi"])

operm <- scan1perm(pr.qc, gm$covar["ici.sick_vs_ici.eoi"], model="binary", addcovar=addcovar, n_perm=10)
...
#out <- scan1(pr.qc, covars["ici.sick_vs_ici.eoi"], Xcovar=Xcovar, model="binary")
out <- scan1(pr.qc, covars["ici.sick_vs_ici.eoi"], model="binary", adcovar=addcovar)
...
peaks <- find_peaks(out, gm$gmap, threshold=summary(operm,alpha=0.05), thresholdX = summary(operm,alpha=0.05), peakdrop=3, drop=1.5)
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici-sick.vs.ici-eoi_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_7.batches_myo_mis.csv"), quote=F)
  ...
  write.csv(genesgss, file=paste0("data/ici-sick.vs.ici-eoi_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_7.batches_myo_mis.csv"), quote=F)
...
#  chr = i
#  blup <- scan1blup(pr.qc[,chr], covars["ici.sick_vs_ici.eoi"])
#  write.csv(as.data.frame(blup), paste0("data/ici-sick.vs.ici-eoi_blup.qc_chr-",chr,"_snpsqc_7.batches_myo_mis.csv"), quote=F)
#}
...
#  chr = i
#  blup <- scan1blup(pr.full[,chr], gm.full$covar["ici.sick_vs_ici.eoi"])
#  write.csv(as.data.frame(blup), paste0("data/ici-sick.vs.ici-eoi_blup.full_chr-",chr,"_snpsqc_7.batches_myo_mis.csv"), quote=F)
#}

#save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici-sick.vs.ici-eoi_scanone_snpsqc_7.batches_myo_mis.Rdata")
...
#phenos$sexn = ifelse(phenos$sex=="M", 1,
#              ifelse(phenos$sex=="F", 0, ''))
...
qtl.d2 <- merge(qtl.d1, phenos[,c("id","ici.sick_vs_ici.eoi","sex","age.of.onset","Histology.Score")], by.x=c("ID"), by.y=c("id"), all=T)
qtl.d2 <- qtl.d2[order(qtl.d2$index),]
qtl.d3 <- qtl.d2[c(1,(ncol(qtl.d2)-3),(ncol(qtl.d2)-2),(ncol(qtl.d2)-1),ncol(qtl.d2),2:(ncol(qtl.d2)-5))]
...
write.csv(qtl.d3,"data/ici-sick.vs.ici-eoi_gm_qtl_snpsqc_7.batches_myo_mis.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici-sick.vs.ici-eoi_gm_qtl_snpsqc_7.batches_myo_mis.csv",alleles=c("A","B"))
cross <- qtl::jittermap(cross)

summary(cross)

cross.probs <- qtl::calc.genoprob(cross, error.prob=0.0001)
#sex <- as.numeric(pull.pheno(cross, "sex")=="M") 
addcovar <- pull.pheno(cross, c("age.of.onset","Histology.Score"))

print("method == hk")

scanone.hk <-qtl::scanone(cross.probs, pheno.col="ici.sick_vs_ici.eoi", addcovar = addcovar, model="binary", method="hk")
operm.hk <- qtl::scanone(cross.probs, method = "hk", pheno.col="ici.sick_vs_ici.eoi", addcovar = addcovar, n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
#plot(operm.hk)
...
#scanone.ehk <-qtl::scanone(cross.probs, pheno.col="ici.sick_vs_ici.eoi" , model="binary", method="ehk")
#operm.ehk <- qtl::scanone(cross.probs, method = "ehk", pheno.col="ici.sick_vs_ici.eoi", n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
#plot(operm.ehk)
