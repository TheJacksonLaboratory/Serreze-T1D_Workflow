...
```{r QTL effects chr8, fig.height = 6, fig.width = 9.5, fig.align = "center", echo=TRUE, results="asis"}
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici.vs.pbs_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_dis_no-x_updated_5.batches_mis_peaks.csv"), quote=F)
 ...
 # Plot 3
  ...
  write.csv(genesgss, file=paste0("data/ici.vs.pbs_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_dis_no-x_updated_5.batches_mis_peaks.csv"), quote=F)
...
```{r QTL effects chr19, fig.height = 6, fig.width = 9.5, fig.align = "center", echo=TRUE, results="asis"}
...
#  write.csv(as.data.frame(blup), paste0("data/ici.vs.pbs_blup.qc_chr-",chr,"_snpsqc_dis_no-x_updated_5.batches_mis_peaks.csv"), quote=F)
#}
...
#  write.csv(as.data.frame(blup), paste0("data/ici.vs.pbs_blup.full_chr-",chr,"_snpsqc_dis_no-x_updated_5.batches_mis_peaks.csv"), quote=F)
#}

#save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici.vs.pbs_scanone_snpsqc_dis_no-x_updated_5.batches_mis_peaks.Rdata")
...
qtl.d2 <- merge(qtl.d1, phenos[,c("id","ICI.vs.PBS","sex")], by.x=c("ID"), by.y=c("id"), all=T)
...
write.csv(qtl.d3,"data/ici.vs.pbs_gm_qtl_snpsqc_dis_no-x_updated_5.batches_mis_peaks.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici.vs.pbs_gm_qtl_snpsqc_dis_no-x_updated_5.batches_mis_peaks.csv",alleles=c("A","B"))
...
print("all peaks with a threshold LOD greater than 1.5")
print(as.data.frame(summary(scanone.hk, perms=operm.hk, threshold=1.5, pvalues=TRUE, format="allpeaks")))
...
qc <- c(as.numeric(peaks$chr[1]), as.numeric(peaks$chr[2]))
qp <- c(as.numeric(peaks$pos[1]), as.numeric(peaks$pos[2]))
...
lod.add.1 <- qtl::fitqtl(fitqtl.d1, pheno.col="ICI.vs.PBS", qtl, formula=y~Q1+Q2,method="hk", model="binary", get.ests=FALSE)
print(summary(lod.add.1))

capture.output(summary(lod.add.1),file="data/ici.vs.pbs_fitqtl_peaks_additive_snpsqc_dis_no-x_updated_5.batches_mis_peaks.txt", quote=F)

# fit model with 2 interacting QTLs interacting
lod.1 <- qtl::fitqtl(fitqtl.d1, pheno.col="ICI.vs.PBS", qtl, formula=y~Q1*Q2, method="hk", model="binary", get.ests=FALSE)
print(summary(lod.1))

capture.output(summary(lod.1),file="data/ici.vs.pbs_fitqtl_peaks_interacting_snpsqc_dis_no-x_updated_5.batches_mis_peaks.txt", quote=F)
...
# additive model
lod.sex.add.1 <- qtl::fitqtl(fitqtl.d1, pheno.col="ICI.vs.PBS", qtl, formula=y~Q1+Q2+Sex, cov=Sex, method="hk", model="binary", get.ests=FALSE)
print(summary(lod.sex.add.1))

capture.output(summary(lod.sex.add.1),file="data/ici.vs.pbs_fitqtl_peaks_sex_additive_snpsqc_dis_no-x_updated_5.batches_mis_peaks.txt", quote=F)

#interactive model

lod.sex.1 <- qtl::fitqtl(fitqtl.d1, pheno.col="ICI.vs.PBS", qtl, formula=y~Q1*Q2*Sex, cov=Sex, method="hk", model="binary", get.ests=FALSE)
print(summary(lod.sex.1))

capture.output(summary(lod.sex.1),file="data/ici.vs.pbs_fitqtl_peaks_sex_interacting_snpsqc_dis_no-x_updated_5.batches_mis_peaks.txt", quote=F)
