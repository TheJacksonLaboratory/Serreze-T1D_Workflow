...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici-early.vs.pbs_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_5.batches_mis.csv"), quote=F)
 ...
 # Plot 3
  ...
  write.csv(genesgss, file=paste0("data/ici-early.vs.pbs_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_5.batches_mis.csv"), quote=F)
...
for (i in names(pr.qc)){
  ...
  write.csv(as.data.frame(blup), paste0("data/ici-early.vs.pbs_blup.qc_chr-",chr,"_5.batches_mis.csv"), quote=F)
}
...
for (i in names(pr.full)){
  ...
  write.csv(as.data.frame(blup), paste0("data/ici-early.vs.pbs_blup.full_chr-",chr,"_5.batches_mis.csv"), quote=F)
}

save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici-early.vs.pbs_scanone_5.batches_mis.Rdata")
...
write.csv(qtl.d3,"data/ici-early.vs.pbs_gm_qtl_5.batches_mis.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici-early.vs.pbs_gm_qtl_5.batches_mis.csv",alleles=c("A","B"))
...
ymx <- 14
plot(scanone.hk, bandcol = "grey90",lty=1, cex=1, col = "slateblue", ylim=c(0, ymx+0.5))
title(main = paste0(colnames(out), " [positions in cM]\n(using same scale as ici-early vs. eoi for easier comparison)"))
qtl::add.threshold(scanone.hk,  perms= operm.hk, alpha=0.01, col = 'blue')
qtl::add.threshold(scanone.hk,  perms= operm.hk, alpha=0.05, col = 'red')
qtl::add.threshold(scanone.hk,  perms= operm.hk, alpha=0.1, col = 'purple')
