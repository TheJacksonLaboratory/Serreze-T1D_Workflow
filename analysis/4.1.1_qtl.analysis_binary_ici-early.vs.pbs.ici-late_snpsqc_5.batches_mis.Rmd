---
title: "QTL Analysis - binary [ICI-Early vs PBS & ICI-Late ]"
...
#           LOD.Difference = ICI.Early.vs.PBS_ICI.Late - dplyr::lag(ICI.Early.vs.PBS_ICI.Late), # Difference in bw between weeks
#           Percent.LOD.Difference = (LOD.Difference /ICI.Early.vs.PBS_ICI.Late) * 100
#           )
...
#mean = mean(out.chr.flw$ICI.Early.vs.PBS_ICI.Late)
#sd = sd(out.chr.flw$ICI.Early.vs.PBS_ICI.Late)
#out.chr.flw$possible.issues <- ifelse(out.chr.flw$ICI.Early.vs.PBS_ICI.Late >= mean+3*sd | out.chr.flw$ICI.Early.vs.PBS_ICI.Late <= mean-3*sd, 1, 0)
...
#write.csv(as.data.frame(all.bad.markers),"data/ici-early.vs.pbs.ici-late_bad.markers_snpsqc_5.batches_mis.csv", row.names=F, quote=F)
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici-early.vs.pbs.ici-late_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_5.batches_mis.csv"), quote=F)
 ...
 # Plot 3
  ...
  write.csv(genesgss, file=paste0("data/ici-early.vs.pbs.ici-late_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_5.batches_mis.csv"), quote=F)
...
for (i in names(pr.qc)){
  chr = i
  blup <- scan1blup(pr.qc[,chr], gm$covar["ICI.Early.vs.PBS_ICI.Late"])
  write.csv(as.data.frame(blup), paste0("data/ici-early.vs.pbs.ici-late_blup.qc_chr-",chr,"_snpsqc_5.batches_mis.csv"), quote=F)
}
...
for (i in names(pr.full)){
  chr = i
  blup <- scan1blup(pr.full[,chr], gm.full$covar["ICI.Early.vs.PBS_ICI.Late"])
  write.csv(as.data.frame(blup), paste0("data/ici-early.vs.pbs.ici-late_blup.full_chr-",chr,"_snpsqc_5.batches_mis.csv"), quote=F)
}

save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici-early.vs.pbs.ici-late_scanone_snpsqc_5.batches_mis.Rdata")
...
qtl.d2 <- merge(qtl.d1, phenos[,c("id","ICI.Early.vs.PBS_ICI.Late","sex")], by.x=c("ID"), by.y=c("id"), all=T)
...
write.csv(qtl.d3,"data/ici-early.vs.pbs.ici-late_gm_qtl_snpsqc_5.batches_mis.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici-early.vs.pbs.ici-late_gm_qtl_snpsqc_5.batches_mis.csv",alleles=c("A","B"))
