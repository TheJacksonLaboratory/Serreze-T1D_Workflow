---
title: "QTL Analysis - rz transformed age of onset (rz.age) [ICI vs PBS] (corrected phenotype & no outliers removed)"
...
#covars <- subset(covars, covars$rz.age!='')
nrow(covars)
table(covars$group)
...
gf_mar_probs <- calc_geno_freq(pr.qc.ids, by = "marker", omit_x = FALSE)
#gn_mar_probs <- calc_geno_freq(probs, by = "individual", omit_x = FALSE)

#gf_mar <- gf_mar[gf_mar[,2] != "NaN",]
gf_mar <- rbind(gf_mar_probs$A[,1:2], gf_mar_probs$X[,1:2])
