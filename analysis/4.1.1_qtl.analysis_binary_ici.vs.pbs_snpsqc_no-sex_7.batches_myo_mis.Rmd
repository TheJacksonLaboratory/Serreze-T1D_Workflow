...
covars <- read_csv("data/covar_corrected_ici.vs.pbs_7.batches_myo_mis.csv")
...
#dismark <- read.csv("data/ici.vs.pbs_marker.freq_low.geno.freq.removed_geno.ratio_7.batches_myo_mis.csv")
#nrow(dismark)
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici.vs.pbs_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_7.batches_myo_mis.csv"), quote=F)
  ...
  write.csv(genesgss, file=paste0("data/ici.vs.pbs_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_7.batches_myo_mis.csv"), quote=F)
...
#  chr = i
#  blup <- scan1blup(pr.qc[,chr], covars["ici_vs_pbs"])
#  write.csv(as.data.frame(blup), paste0("data/ici.vs.pbs_blup.qc_chr-",chr,"_snpsqc_7.batches_myo_mis.csv"), quote=F)
#}
...
#  chr = i
#  blup <- scan1blup(pr.full[,chr], gm.full$covar["ici_vs_pbs"])
#  write.csv(as.data.frame(blup), paste0("data/ici.vs.pbs_blup.full_chr-",chr,"_snpsqc_7.batches_myo_mis.csv"), quote=F)
#}

#save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici.vs.pbs_scanone_snpsqc_7.batches_myo_mis.Rdata")
...
qtl.d2 <- merge(qtl.d1, phenos[,c("id","ici_vs_pbs","sex","age.of.onset","Histology.Score")], by.x=c("ID"), by.y=c("id"), all=T)
...
write.csv(qtl.d3,"data/ici.vs.pbs_gm_qtl_snpsqc_7.batches_myo_mis.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici.vs.pbs_gm_qtl_snpsqc_7.batches_myo_mis.csv",alleles=c("A","B"))
...
scanone.hk <-qtl::scanone(cross.probs, pheno.col="ici_vs_pbs" , addcovar = addcovar, model="binary", method="hk")
operm.hk <- qtl::scanone(cross.probs, method = "hk", pheno.col="ici_vs_pbs", addcovar = addcovar, n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
#plot(operm.hk)
...
#scanone.ehk <-qtl::scanone(cross.probs, pheno.col="ici_vs_pbs" , model="binary", method="ehk")
#operm.ehk <- qtl::scanone(cross.probs, method = "ehk", pheno.col="ici_vs_pbs", n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
#plot(operm.ehk)
