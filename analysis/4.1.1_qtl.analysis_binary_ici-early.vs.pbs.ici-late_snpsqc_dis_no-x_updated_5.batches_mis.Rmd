...
covars <- read_csv("data/covar_corrected_ici-early.vs.pbs.ici-late_5.batches_mis.csv")
#removing any missing info
covars <- subset(covars, covars$ICI.Early.vs.PBS_ICI.Late!='')
nrow(covars)
table(covars$group)
...
dismark <- read.csv("data/ici-early.vs.pbs.ici-late_marker.freq_low.geno.freq.removed_geno.ratio_5.batches_mis.csv")
nrow(dismark)
...
operm <- scan1perm(pr.qc, gm$covar["ICI.Early.vs.PBS_ICI.Late"], model="binary", n_perm=1000, perm_Xsp=TRUE, chr_lengths=chr_lengths(gm$gmap))
...
out <- scan1(pr.qc, gm$covar["ICI.Early.vs.PBS_ICI.Late"], model="binary")
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici-early.vs.pbs.ici-late_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_dis_no-x_updated_5.batches_mis.csv"), quote=F)
 ...
 # Plot 3
  ...
  write.csv(genesgss, file=paste0("data/ici-early.vs.pbs.ici-late_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_dis_no-x_updated_5.batches_mis.csv"), quote=F)
...
for (i in names(pr.qc)){
  ...
  write.csv(as.data.frame(blup), paste0("data/ici-early.vs.pbs.ici-late_blup.qc_chr-",chr,"_snpsqc_dis_no-x_updated_5.batches_mis.csv"), quote=F)
}
...
for (i in names(pr.full)){
  ...
  write.csv(as.data.frame(blup), paste0("data/ici-early.vs.pbs.ici-late_blup.full_chr-",chr,"_snpsqc_dis_no-x_updated_5.batches_mis.csv"), quote=F)
}

save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici-early.vs.pbs.ici-late_scanone_snpsqc_dis_no-x_updated_5.batches_mis.Rdata")
...
write.csv(qtl.d3,"data/ici-early.vs.pbs.ici-late_gm_qtl_snpsqc_dis_no-x_updated_5.batches_mis.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici-early.vs.pbs.ici-late_gm_qtl_snpsqc_dis_no-x_updated_5.batches_mis.csv",alleles=c("A","B"))
...
operm.hk <- qtl::scanone(cross.probs, method = "hk", pheno.col="ICI.Early.vs.PBS_ICI.Late", n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
plot(operm.hk)
