...
#           LOD.Difference = ICI.vs.PBS - dplyr::lag(ICI.vs.PBS), # Difference in bw between weeks
#           Percent.LOD.Difference = (LOD.Difference /ICI.vs.PBS) * 100
#           )
...
#mean = mean(out.chr.flw$ICI.vs.PBS)
#sd = sd(out.chr.flw$ICI.vs.PBS)
#out.chr.flw$possible.issues <- ifelse(out.chr.flw$ICI.vs.PBS >= mean+3*sd | out.chr.flw$ICI.vs.PBS <= mean-3*sd, 1, 0)
...
#write.csv(as.data.frame(all.bad.markers),"data/ici.vs.pbs_bad.markers_snpsqc_5.batches_mis.csv", row.names=F, quote=F)
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici.vs.pbs_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_5.batches_mis.csv"), quote=F)
 ...
 # Plot 3
  ...
  write.csv(genesgss, file=paste0("data/ici.vs.pbs_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_5.batches_mis.csv"), quote=F)
...
#  write.csv(as.data.frame(blup), paste0("data/ici.vs.pbs_blup.qc_chr-",chr,"_snpsqc_5.batches_mis.csv"), quote=F)
#}
...
#  write.csv(as.data.frame(blup), paste0("data/ici.vs.pbs_blup.full_chr-",chr,"_snpsqc_5.batches_mis.csv"), quote=F)
#}

#save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici.vs.pbs_scanone_snpsqc_5.batches_mis.Rdata")
...
write.csv(qtl.d3,"data/ici.vs.pbs_gm_qtl_snpsqc_5.batches_mis.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici.vs.pbs_gm_qtl_snpsqc_5.batches_mis.csv",alleles=c("A","B"))
