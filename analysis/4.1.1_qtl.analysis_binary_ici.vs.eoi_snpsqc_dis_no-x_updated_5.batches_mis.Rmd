...
+ binary phenotype (therefore no kinship or outliers)
+ no Xcovar
...
covars <- read_csv("data/covar_corrected_ici.vs.eoi_5.batches_mis.csv")
...
dismark <- read.csv("data/ici.vs.eoi_marker.freq_low.geno.freq.removed_geno.ratio_5.batches_mis.csv")
nrow(dismark)
...
```{r prelim, fig.height = 6, fig.width = 9.5, fig.align = "center", echo=FALSE, include=FALSE}

### Prelim qtl scan to remove problematic variants only identified when viewing results
...
#out.chr <- merge(out, gmapdf, by=c("marker"), all.x=T)
#out.chr <- out.chr[order(out.chr$index),]#

#ind.fl.chr <- list()
#for(i in unique(out.chr$chr)){
#    chr <- out.chr[out.chr$chr==i,]
#    chr.flw = chr %>%
#    # first sort by Age
#    #arrange(Age.Week.Universal) %>%
#    mutate(Diff_pos = pos - dplyr::lag(pos),  # Difference in weeks
#           LOD.Difference = ICI.vs.EOI - dplyr::lag(ICI.vs.EOI), # Difference in bw between weeks
#           Percent.LOD.Difference = (LOD.Difference /ICI.vs.EOI) * 100
#           )
#    chr.flw[is.na(chr.flw$LOD.Difference),]$LOD.Difference <- 0
#    chr.flw[is.na(chr.flw$Percent.LOD.Difference),]$Percent.LOD.Difference <- 0#

#    ind.fl.chr[[i]] <- chr.flw
#}

#out.chr.flw <- do.call("rbind",ind.fl.chr)
#mean = mean(out.chr.flw$ICI.vs.EOI)
#sd = sd(out.chr.flw$ICI.vs.EOI)
#out.chr.flw$possible.issues <- ifelse(out.chr.flw$ICI.vs.EOI >= mean+3*sd | out.chr.flw$ICI.vs.EOI <= mean-3*sd, 1, 0)
#poss.issues <- out.chr.flw[which(out.chr.flw$possible.issues == 1),]
#bad.chr <- as.data.frame(table(poss.issues$chr))
#bad.chr <- bad.chr[bad.chr$Freq <=10,]
#bad.markers <- list()
#for(i in 1:nrow(bad.chr)){
#    chrom <- as.character(bad.chr[i,1])
#    bad.mar <- poss.issues[poss.issues$chr==chrom,]
#    bad.markers[[i]] <- bad.mar
#}

#all.bad.markers<- do.call("rbind",bad.markers)$marker
#print(paste0(length(all.bad.markers), " bad markers indeitified through qtl scan.  Removing now."))

#write.csv(as.data.frame(all.bad.markers),"data/ici.vs.eoi_bad.markers_snpsqc_dis_no-x_updated_5.batches_mis.csv", row.names=F, quote=F)

#gm_qc <- drop_markers(gm, all.bad.markers)
#gm_qc <- drop_nullmarkers(gm_qc)
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici.vs.eoi_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_dis_no-x_updated_5.batches_mis.csv"), quote=F)
 ...
 # Plot 3
  ...
  write.csv(genesgss, file=paste0("data/ici.vs.eoi_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_dis_no-x_updated_5.batches_mis.csv"), quote=F)
...
#  chr = i
#  blup <- scan1blup(pr.qc[,chr], gm$covar["ICI.vs.EOI"])
#  write.csv(as.data.frame(blup), paste0("data/ici.vs.eoi_blup.qc_chr-",chr,"_snpsqc_dis_no-x_updated_5.batches_mis.csv"), quote=F)
#}
...
#  chr = i
#  blup <- scan1blup(pr.full[,chr], gm.full$covar["ICI.vs.EOI"])
#  write.csv(as.data.frame(blup), paste0("data/ici.vs.eoi_blup.full_chr-",chr,"_snpsqc_dis_no-x_updated_5.batches_mis.csv"), quote=F)
#}

#save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici.vs.eoi_scanone_snpsqc_dis_no-x_updated_5.batches.Rdata")
...
qtl.d2 <- merge(qtl.d1, phenos[,c("id","ICI.vs.EOI","sex")], by.x=c("ID"), by.y=c("id"), all=T)
qtl.d2 <- qtl.d2[order(qtl.d2$index),]
qtl.d3 <- qtl.d2[c(1,(ncol(qtl.d2)-1),ncol(qtl.d2),2:(ncol(qtl.d2)-3))]
qtl.d3[!is.na(qtl.d3$sex),]$sex <- 0

#putting missing in csv files
qtl.d3[1:2,1:3] <- ""

write.csv(qtl.d3,"data/ici.vs.eoi_gm_qtl_snpsqc_dis_no-x_updated_5.batches_mis.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici.vs.eoi_gm_qtl_snpsqc_dis_no-x_updated_5.batches_mis.csv",alleles=c("A","B"))
...
operm.hk <- qtl::scanone(cross.probs, method = "hk", pheno.col="ICI.vs.EOI", n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
plot(operm.hk)
