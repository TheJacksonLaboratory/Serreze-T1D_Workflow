---
title: "QTL Analysis - binary [ICI-Early vs PBS]"
...
We will load the data and subset indivials out that are in the groups of interest.  We will create a binary phenotype from this  (PBS ==0, ICI-Early == 1).
...
#           LOD.Difference = ICI_Early.vs.PBS - dplyr::lag(ICI_Early.vs.PBS), # Difference in bw between weeks
#           Percent.LOD.Difference = (LOD.Difference /ICI_Early.vs.PBS) * 100
#           )
...
#mean = mean(out.chr.flw$ICI_Early.vs.PBS)
#sd = sd(out.chr.flw$ICI_Early.vs.PBS)
#out.chr.flw$possible.issues <- ifelse(out.chr.flw$ICI_Early.vs.PBS >= mean+3*sd | out.chr.flw$ICI_Early.vs.PBS <= mean-3*sd, 1, 0)
...
#write.csv(as.data.frame(all.bad.markers),"data/ici-early.vs.pbs_bad.markers_snpsqc_5.batches_mis.csv", row.names=F, quote=F)
...
plot_lod<-function(out,map){
  for (i in 1:dim(out)[2]){
    ...
    plot(out, map, lodcolumn=i, col="slateblue", ylim=c(0, ymx+0.5))
    #legend("topright", lwd=2, colnames(out)[i], bg="gray90")
    title(main = paste0(colnames(out)[i], " [positions in cM] \n(using same scale as eoi vs ici-early for easier comparison)"))
    add_threshold(map,  summary(operm, alpha=0.1), col = 'purple')
    add_threshold(map,  summary(operm, alpha=0.05), col = 'red')
    add_threshold(map,  summary(operm, alpha=0.01), col = 'blue')
...
for (i in names(pr.qc)){
  chr = i
  blup <- scan1blup(pr.qc[,chr], gm$covar["ICI_Early.vs.PBS"])
  write.csv(as.data.frame(blup), paste0("data/ici-early.vs.pbs_blup.qc_chr-",chr,"_snpsqc_5.batches_mis.csv"), quote=F)
}
...
for (i in names(pr.full)){
  chr = i
  blup <- scan1blup(pr.full[,chr], gm.full$covar["ICI_Early.vs.PBS"])
  write.csv(as.data.frame(blup), paste0("data/ici-early.vs.pbs_blup.full_chr-",chr,"_snpsqc_5.batches_mis.csv"), quote=F)
}

save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici-early.vs.pbs_scanone_snpsqc_5.batches_mis.Rdata")
...
qtl.d2 <- merge(qtl.d1, phenos[,c("id","ICI_Early.vs.PBS","sex")], by.x=c("ID"), by.y=c("id"), all=T)
...
write.csv(qtl.d3,"data/ici-early.vs.pbs_gm_qtl_snpsqc_5.batches_mis.csv", quote=F, row.names=F)
