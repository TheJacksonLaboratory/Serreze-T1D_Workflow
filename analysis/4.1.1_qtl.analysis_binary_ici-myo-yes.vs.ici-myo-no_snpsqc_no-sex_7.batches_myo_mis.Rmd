---
title: "QTL Analysis - binary [ICI Myocarditis Yes vs ICI Myocarditis No]"
...
#dismark <- read.csv("data/ici-myo-yes.vs.ici-myo-no_marker.freq_low.geno.freq.removed_geno.ratio_7.batches_myo_mis.csv")
#nrow(dismark)
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/ici-myo-yes.vs.ici-myo-no_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_7.batches_myo_mis.csv"), quote=F)
  ...
  write.csv(genesgss, file=paste0("data/ici-myo-yes.vs.ici-myo-no_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_7.batches_myo_mis.csv"), quote=F)
...
#  chr = i
#  blup <- scan1blup(pr.qc[,chr], covars["ici.myo.yes_vs_ici.myo.no"])
#  write.csv(as.data.frame(blup), paste0("data/ici-myo-yes.vs.ici-myo-no_blup.qc_chr-",chr,"_snpsqc_7.batches_myo_mis.csv"), quote=F)
#}
...
#  chr = i
#  blup <- scan1blup(pr.full[,chr], gm.full$covar["ici.myo.yes_vs_ici.myo.no"])
#  write.csv(as.data.frame(blup), paste0("data/ici-myo-yes.vs.ici-myo-no_blup.full_chr-",chr,"_snpsqc_7.batches_myo_mis.csv"), quote=F)
#}

#save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/ici-myo-yes.vs.ici-myo-no_scanone_snpsqc_7.batches_myo_mis.Rdata")
...
qtl.d2 <- merge(qtl.d1, phenos[,c("id","ici.myo.yes_vs_ici.myo.no","sex","Histology.Score")], by.x=c("ID"), by.y=c("id"), all=T)
...
write.csv(qtl.d3,"data/ici-myo-yes.vs.ici-myo-no_gm_qtl_snpsqc_7.batches_myo_mis.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/ici-myo-yes.vs.ici-myo-no_gm_qtl_snpsqc_7.batches_myo_mis.csv",alleles=c("A","B"))
...
scanone.hk <-qtl::scanone(cross.probs, pheno.col="ici.myo.yes_vs_ici.myo.no" , addcovar = addcovar, model="binary", method="hk")
operm.hk <- qtl::scanone(cross.probs, method = "hk", pheno.col="ici.myo.yes_vs_ici.myo.no", addcovar = addcovar, n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
#plot(operm.hk)
...
#scanone.ehk <-qtl::scanone(cross.probs, pheno.col="ici.myo.yes_vs_ici.myo.no" , model="binary", method="ehk")
#operm.ehk <- qtl::scanone(cross.probs, method = "ehk", pheno.col="ici.myo.yes_vs_ici.myo.no", n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
#plot(operm.ehk)
