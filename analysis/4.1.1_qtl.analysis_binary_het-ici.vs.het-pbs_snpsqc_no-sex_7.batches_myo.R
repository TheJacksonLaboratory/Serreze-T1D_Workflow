---
title: "QTL Analysis - binary [Hetero: ICI vs PBS]"
...
covars <- read_csv("data/covar_corrected_het-ici.vs.het-pbs_7.batches_myo.csv")
...
#dismark <- read.csv("data/het-ici.vs.het-pbs_marker.freq_low.geno.freq.removed_geno.ratio_7.batches_myo.csv")
#nrow(dismark)
...
# Plot 2
  ...
  write.csv(as.data.frame(blup_sub), paste0("data/het-ici.vs.het-pbs_blup_sub_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_7.batches_myo.csv"), quote=F)
  ...
  write.csv(genesgss, file=paste0("data/het-ici.vs.het-pbs_genes_chr-",chr,"_peak.marker-",peaks$marker[i],"_lod.drop-1.5_snpsqc_7.batches_myo.csv"), quote=F)
...
#  chr = i
#  blup <- scan1blup(pr.qc[,chr], covars["het.ici_vs_het.pbs"])
#  write.csv(as.data.frame(blup), paste0("data/het-ici.vs.het-pbs_blup.qc_chr-",chr,"_snpsqc_7.batches_myo.csv"), quote=F)
#}
...
#  chr = i
#  blup <- scan1blup(pr.full[,chr], gm.full$covar["het.ici_vs_het.pbs"])
#  write.csv(as.data.frame(blup), paste0("data/het-ici.vs.het-pbs_blup.full_chr-",chr,"_snpsqc_7.batches_myo.csv"), quote=F)
#}

#save(out, operm, gm.full, gm, pr.qc, pr.full, file="data/het-ici.vs.het-pbs_scanone_snpsqc_7.batches_myo.Rdata")
...
qtl.d2 <- merge(qtl.d1, phenos[,c("id","het.ici_vs_het.pbs","sex","age.of.onset","Histology.Score")], by.x=c("ID"), by.y=c("id"), all=T)
...
write.csv(qtl.d3,"data/het-ici.vs.het-pbs_gm_qtl_snpsqc_7.batches_myo.csv", quote=F, row.names=F)
...
cross <- qtl::read.cross("csv", file = "data/het-ici.vs.het-pbs_gm_qtl_snpsqc_7.batches_myo.csv",alleles=c("A","B"))
...
scanone.hk <-qtl::scanone(cross.probs, pheno.col="het.ici_vs_het.pbs" , addcovar = addcovar, model="binary", method="hk")
operm.hk <- qtl::scanone(cross.probs, method = "hk", pheno.col="het.ici_vs_het.pbs", addcovar = addcovar, n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
#plot(operm.hk)
...
#scanone.ehk <-qtl::scanone(cross.probs, pheno.col="het.ici_vs_het.pbs" , model="binary", method="ehk")
#operm.ehk <- qtl::scanone(cross.probs, method = "ehk", pheno.col="het.ici_vs_het.pbs", n.perm = 1000, perm.Xsp = TRUE, model="binary", verbose=FALSE)
#plot(operm.ehk)
